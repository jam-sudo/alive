...
1. **It never raises during terminal assembly.** An inconsistent ``(band_passes, flip)`` pair is
   ...
   inconsistency as a marker instead; ``durable`` refuses that marker as a publishable artifact
...
def test_the_renderer_module_is_a_leaf_of_the_terminal_stack():
    ...
    ``phase2b`` already imports ``durable`` (``phase2b.py`` module scope), so a renderer living in
    ``phase2b`` cannot be imported by ``durable`` -- measured as
    ``ImportError: ... partially initialized module 'alive.compose.phase2b'``. This pins the
    property that made a new module necessary, at the source level where a future edit would
    break it.
    """
    source = pathlib.Path("src/alive/compose/headline.py").read_text(encoding="utf-8")
