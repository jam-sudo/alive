...
_DEFAULT_PROBE_A = _probe_a_snapshot()
_REGISTRATION_SHA = _DEFAULT_PROBE_A.registration_sha256
_VERIFICATION_SHA = _DEFAULT_PROBE_A.verification_sha256
...
def test_control_reference_rows_are_accepted_but_not_measured(tmp_path):
    ...
    report = module.measure_approximation_bias_v3(
        ...
        probe_a_registration_sha256=_REGISTRATION_SHA,
        probe_a_verification_sha256=_VERIFICATION_SHA,
    )
...
def test_report_has_v2_schema_and_strata(tmp_path):
    ...
    assert prov["probe_a_registration_sha256"] == _REGISTRATION_SHA
    assert prov["probe_a_verification_sha256"] == _VERIFICATION_SHA
...
def test_direct_producer_requires_matching_external_registration_pin(tmp_path):
    """Admission bytes cannot nominate their own allegedly preregistered tolerance."""
    module = _load_metric_module()
    kwargs = _full_report_fixture(tmp_path)
...
def test_forged_admission_cannot_widen_registered_tolerance(tmp_path):
    module = _load_metric_module()
    kwargs = _full_report_fixture(tmp_path)
    original = kwargs["probe_a_evidence"]
    forged = json.loads(original.evidence_bytes)
    forged["output_bridge"]["tolerance"] = 1e100
    forged["output_bridge"]["max_abs_error"] = 1e99
    body = {key: value for key, value in forged.items() if key != "self_checksum"}
    forged["self_checksum"] = self_checksum(body)
    forged_bytes = (canonical_json(forged) + "\n").encode("utf-8")
    kwargs["probe_a_evidence"] = ProbeAEvidence(
        content_sha256=sha256_bytes(forged_bytes),
        evidence_bytes=forged_bytes,
        registration_sha256=original.registration_sha256,
        registration_bytes=original.registration_bytes,
...
def _write_probe_a_evidence(tmp_path: Path, status: str, *, git_commit: str = "b" * 40) -> Path:
    ...
    snapshot = _probe_a_snapshot(git_commit=git_commit)
    (tmp_path / "probe_a_registration.json").write_bytes(snapshot.registration_bytes)
    (tmp_path / "verify.json").write_bytes(snapshot.verification_bytes)
    if status == "pass":
        path.write_bytes(snapshot.evidence_bytes)
    else:
        path.write_text(canonical_json({"status": status}) + "\n", encoding="utf-8")
    return path
...
def _main_cli_argv(fixture: dict, probe_a_evidence_path: Path, out_path: Path) -> list[str]:
    return [
        ...
        str(probe_a_evidence_path.with_name("probe_a_registration.json")),
        "--probe-a-registration-sha256",
        sha256_file(probe_a_evidence_path.with_name("probe_a_registration.json")),
        "--probe-a-verification",
        str(probe_a_evidence_path.with_name("verify.json")),
        "--probe-a-verification-sha256",
        sha256_file(probe_a_evidence_path.with_name("verify.json")),
        "--fit-role-artifact",
...
def test_probe_a_bare_pass_is_not_admission_grade(tmp_path):
    module = _load_metric_module()
    fixture = _write_main_cli_fixture(tmp_path)
    probe_a_evidence_path = _write_probe_a_evidence(tmp_path, "failed")
    probe_a_evidence_path.write_text(canonical_json({"status": "pass"}) + "\n", encoding="utf-8")
    out_path = tmp_path / "report.json"
...
def test_probe_a_unknown_or_absent_status_fails_closed(tmp_path, evidence):
    ...
    fixture = _write_main_cli_fixture(tmp_path)
    probe_a_evidence_path = _write_probe_a_evidence(tmp_path, "failed")
    probe_a_evidence_path.write_text(canonical_json(evidence) + "\n", encoding="utf-8")
    out_path = tmp_path / "report.json"
