...
combo zero-shot generalization within tolerance, (c) graceful degradation when
rank-deficient, (d) noise-robust false-GI guard (no true GI + noise must not
hallucinate real-GI-magnitude interactions), (e) k vs |Cal| frontier.
...
(`configs/compose_k562_v1_phase1.yaml`):
  ...
  * Held-out (double-unseen) generalization is asserted LOOSER than the
    coefficient tolerance (``2 * RECOVERY_REL_ERR_TOL``), per §3.4: a zero-shot
    pair prediction is a strictly harder, noise-amplified target. Empirically it
    sits ~0.01 across registered seeds, so the bound is generous but principled.
  * False-GI is asserted as a RATIO, not a magic absolute: spurious recovered-GI
    ...
    spurious norm (~0.08-0.6 across seeds/noise) would VIOLATE the config's
    `false_gi_tol: 0.05` on several seeds, which is exactly why the honest guard
    is scale-relative. Observed worst ratio is ~0.05; the 0.1 margin leaves ~2x
    headroom while still catching real-GI-magnitude hallucination.
"""
...
def test_held_out_noisy_generalization():
    # (b) double-unseen: the NOISY fit predicts a held-out (disjoint) pair vs ground
    # truth. Must be finite, noise-driven, and within the looser held-out tolerance.
    r = run_recovery(n_genes=40, p=5, rank=2, n_pairs=60, noise_sd=0.05, seed=1, k=4)
...
def test_false_gi_guard_noise_robust():
    ...
    r = run_recovery(n_genes=40, p=5, rank=0, n_pairs=60, noise_sd=0.05, seed=2, k=4)
...
def test_false_gi_guard_holds_across_noise_and_seeds():
    ...
    for seed in (11, 23, 37):
        for noise_sd in (0.05, 0.1, 0.2):
            r = run_recovery(
                n_genes=40, p=5, rank=0, n_pairs=60, noise_sd=noise_sd, seed=seed, k=4
            )
            assert r.false_gi_norm_noisy < FALSE_GI_RATIO_MARGIN * r.genuine_gi_norm
