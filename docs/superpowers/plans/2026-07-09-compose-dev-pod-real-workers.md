...
- [ ] **Acceptance:** both imports succeed; record the resolved package revisions (for Task 2.1's dependency-lock pin). If a revision differs from the committed lock, MODIFY the lock file and commit `fix(compose): pin confirmed gears/cpa env revisions`.
