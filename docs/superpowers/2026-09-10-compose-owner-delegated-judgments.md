...
**실측.** ledger 는 4열이고, 그 소비자는 앞쪽 두 열만 읽는다 — 뒤에 열을 덧붙이는 것은 소비자를
바꾸지 않는 **호환 변경**이다. 현재 ledger 는 한 항목이 어느 실행 슬롯에 속했는지, 어느 실행이 그것을
썼는지를 담지 않으므로, 같은 날 여러 번 기록된 항목을 사후에 구분할 수 없다.
