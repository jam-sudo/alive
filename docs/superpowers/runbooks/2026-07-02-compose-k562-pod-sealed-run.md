...
| 대응 | exception | 조치 |
|------|-----------|------|
| **A. 정지·보고 (재실행 금지)** | `OutcomeLeakageError` · `LeakageError` · `ComposeSealingError` · `FitRoleArtifactError` · `TerminalError` · **seal 증거가 없는 `phase2b`/`recover`의 `RunDirStateError`** | leakage/seal 경계 위반이다. ⚑ `phase2b`에서 run_dir에 terminal이나 소진된 seal audit이 있으면 driver가 **exit 30 + "`recover`를 쓰라"** 로 라우팅한다(그 경우는 D가 아니라 recover 절차). A는 그 증거가 **없는데도** roster를 위반한 경우다. **재실행하지 않는다.** artifact를 하나도 지우지 말고 보존한 뒤 owner에게 보고한다. ⚑ `RunDirStateError`가 `phase2b`에서 말하는 "forbidden basename" 목록에는 `audit.jsonl`·terminal이 들어갈 수 있는데, 그 조건을 "고친다"는 것은 **write-once seal 기록을 지운다**는 뜻이므로 절대 하지 않는다. `TerminalError`가 exit 10으로 도달하는 경우는 `Phase2bTerminal.acquire()`의 lock 점유 또는 run_dir 부재이며, audit에 기록이 있다는 거부는 `_seal_consumed` 안쪽이라 30으로 나간다 |
| **B. 상류 재생성 후 재실행** | `HashMismatchError` · `ConfigContractError` · `InputContractError` · `ProvenanceError` · `AssemblerError` · `Phase2ConfigError` · `ActivationEvidenceError` · `ApproximationBiasValidationError` · `ApproximationBiasDeclarationError` · `DataCardError` · `ScientificModeError` · `FreezeError` · `OOFFoldManifestError` · `Phase2bSubcommandError` · `EstimatorInputError` (⚑ `EstimatorInputError`는 factor bank/eps에 non-finite 값이나 정렬 오류가 있다는 뜻이다. 상류에서 아무도 finiteness를 검사하지 않으므로 여기가 첫 관문이며, PREPARE 산출물을 다시 만들어야 한다.) | artifact·config·evidence가 기록된 identity와 불일치한다. **먼저 현 artifact를 보존한다**(`HashMismatchError`는 변조 신호일 수 있고 상류 재생성은 그 증거를 지운다). 그 뒤 PREPARE 산출물을 다시 만든다. `Phase2bSubcommandError`는 sealed-source digest 불일치를 포함하는데, 이는 환경 문제가 아니라 lineage 사건이다. "상류 재생성"은 **PREPARE 입력에 한정**하며 C의 write-once 삭제 금지 규칙이 여기에도 그대로 적용된다. **config/evidence field를 바꾸면 새 run identity다** |
