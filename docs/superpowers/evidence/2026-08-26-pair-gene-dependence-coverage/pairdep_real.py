...
print(f"실제 headline 그래프: n_pairs={len(edges)} genes={n_genes} "
      f"mean_deg={st['mean_degree']:.2f} share={st['shared_gene_pair_fraction']:.3f}", flush=True)
...
for sigma_gene in (0.0, 0.30, 0.60, 0.90, 1.20):
    ...
    row = {"sigma_gene": sigma_gene, "icc_one_shared_gene": icc, **cov,
           "secs": round(time.time() - t0, 1)}
    out["rows"].append(row)
    print(f"REAL sg={sigma_gene:.2f} icc={icc:.3f}  coverage={cov['coverage']:.4f} "
