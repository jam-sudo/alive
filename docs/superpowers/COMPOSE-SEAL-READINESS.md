...
> **Updated:** 2026-07-12 @ `75c2c17` (branch `compose-scientific-prepare-carrier`)
...
| C | 단일 production driver | ✅ **fixture orchestration merged** (merge commit `1c46708`, 2026-07-09) — T1–T14 완료; whole-branch 2-lens 리뷰(seal-safety + correctness) → Important 2건 fix(phase2b step-5 post-seal raise → exit 30 `6761428`; 미연결 pre-seal pair-index/attestation validator를 preflight에 연결 `4deee1a`); driver 211 / compose 1106 green. **Scientific PREPARE carrier = 🟡 spec+plan ready, 구현 대기(branch `compose-scientific-prepare-carrier`)**: committed `load_run_spec_carrier`는 아직 `mode="scientific"`을 `UnsupportedModeError`로 거부. design spec은 owner deep-revision + code-claim 독립검증(10/11 CONFIRMED, §7.3 incoherent equality 1건 fix) + loop_gate spec-review PASS(iter2) 완료. **10-task TDD 구현 plan 작성 완료** → subagent-driven-development 대기. scope = carrier loader + no-seal assembly test를 sub-project-B 경계까지(§5 `assemble_execution_identity_lock`는 B의 versioned `adapter_version` 전까지 fail-closed), carrier = discriminated `RunSpecCarrier`(mode+6 scientific fields, `__post_init__`), git identity = trusted-repo-root 내부 resolve(no caller-asserted clean), GU roster bind는 defer(§7.3는 release-blocker). seal-run 전 완료 필요; **seal 안 열림** | `specs/2026-07-11-compose-scientific-prepare-carrier-design.md`; `plans/2026-07-12-compose-scientific-prepare-carrier.md`; `specs/2026-07-07-compose-production-driver-design.md` |
