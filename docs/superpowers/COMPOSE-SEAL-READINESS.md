...
> **Updated:** 2026-07-09 @ `4deee1a` (branch `compose-c-driver`)
...
| C | 단일 production driver | 🟡 **in-progress** — branch `compose-c-driver`, T1–T13 + Task 11.5 완료; whole-branch 2-lens 리뷰(seal-safety + correctness) 완료 → **Important 2건 fix 적용**(phase2b step-5 post-seal raise → exit 30 not 10/1; 미연결 pre-seal pair-index/attestation validator를 preflight에 연결), 전체 compose suite green + ruff clean. 잔여: **→main 병합** — 코드는 clean 병합이나 `CLAUDE.md` + 이 인덱스 문서 충돌은 ARS-guard라 **owner reconcile 필요** | `specs/2026-07-07-compose-production-driver-design.md`; `plans/2026-07-08-compose-c-production-driver.md` |
