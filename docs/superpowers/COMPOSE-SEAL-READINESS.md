...
> **Updated:** 2026-07-09 @ `1c46708` (branch `main`)
...
4. **development pod (real workers + evidence 재생성)** — §2.2 real `gears_worker`/`cpa_worker` + GO graph + pinned env 구현·검증·commit; §4 activation-evidence를 **active config(`a4700194…`)로 real Norman data에서 재생성** + null requirement(config `power_status` · gears/cpa `environment_status`/`revision` · GEARS `approximation_bias_report_sha256`) 확립. real fit은 pod-only. *(현재 committed evidence는 pre-activation `d8c65ac4…`/`activation=BLOCKED` → §2.5 재생성 규칙 발효됨.)*
