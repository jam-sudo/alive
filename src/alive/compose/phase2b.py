...
# The branch and the four pre-registered sentences live in the LEAF module `headline` so
# `durable` can re-derive them: `phase2b` already imports `durable` above, so a renderer defined
# here could not be imported there (measured: ImportError, partially initialized module).
# `preregistered_headline_branch` is re-exported unchanged because the pair-dependence decision
# §8 cites the dotted path `alive.compose.phase2b.preregistered_headline_branch`.
...
def _bias_numeric_or_sentinel(value: object, *, path: Path, field: str) -> float | str:
    ...
    ``measure_approximation_bias_v4`` legitimately emits the STRING
...
def _load_approximation_bias_fairness(
    *,
    ...
    ``measure_approximation_bias_v4`` emits them (``fairness_flag``,
